    ...
    def analisar_fuga_bci(texte_emails):
      ...
      clientes_salvos = int(total * 0.02 * 30)
    ...
    # 5. CRÉATION INTERFACE
    demo = gr.Interface(
      fn=analisar_fuga_bci,
      inputs=gr.Textbox(label="Colar emails BCI aqui (1 por linha) ou deixar vazio para usar démo 20 emails", lines=8),
      outputs=[gr.Plot(label="Gráfico para Director BCI"), gr.Textbox(label="Relatório + Recomendação + ROI", lines=15)],
      title="BCI - Detector Fuga Clientes",
      description="Clique 'Submit'. IA analisa em 2s e diz porque clientes fogem + quanto BCI perde.",
      submit_btn="Analisar Agora",
      clear_btn="Limpar"
    )

    # 6. LANCER - IMPORTANT pour HuggingFace
